...
class TableCreator(ABC):
    ...
    def create_table(self) -> bool:
        ...
        try:
            if not self.table_exists():
                self.db_manager._ensure_connection()
                self.db_manager.cursor.execute(self.get_table_definition())
    ...
    def insert_base_data(self) -> bool:
        ...
        try:
            ...
            self.db_manager.connection.commit()
            print(f"✅ Dados base inseridos em {self.table_name}: {len(base_data)} registros")
            return True
        except Exception as e:
    ...
    def recreate_table(self) -> bool:
        ...
        try:
            self.db_manager._ensure_connection()
            self.db_manager.cursor.execute(f"DROP TABLE IF EXISTS {self.table_name}")
            self.db_manager.connection.commit()
