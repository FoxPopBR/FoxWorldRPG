...
class DatabaseManager:
        ...
        try:
            ...
            for category, key, value, data_type in settings:
                ...
                if data_type == "int" and value in ("True", "False", "true", "false"):
                    ...
                    migrated_count += 1
                    print(
                        f"🔧 Migrado: {category}.{key} de '{data_type}:{value}' para 'bool:{new_value}'"
                    )

            if migrated_count > 0:
                self.connection.commit()
                print(
                    f"🎯 Migração concluída: {migrated_count} configurações corrigidas"
                )
    ...
    def delete_all_settings(self, category: str = None):
        ...
        try:
            ...
            if category:
                ...
                print(
                    f"🗑️ Todas as configurações da categoria '{category}' foram deletadas"
                )
            else:
    ...
    def get_all_characters(self) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute("SELECT value FROM settings WHERE category = 'characters'")
            results = cursor.fetchall()
    ...
    def initialize_game_tables(self, force_recreate_static: bool = False) -> bool:
        ...
        try:
            ...
            from src.database.datamaker.game_slots import GameSlotsTable
            from src.database.datamaker.save_slots import SaveSlotsTable
            ...
            table_creators = [
                ...
                GameSlotsTable(self),  # Tabela estática (slots de jogo)
                SaveSlotsTable(self),  # Tabela dinâmica (saves por slot de jogo)
            ]
    ...
    def get_hero_class_by_key(self, class_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute(
                "SELECT * FROM hero_classes WHERE class_key = ?", (class_key,)
            )
            result = cursor.fetchone()
    ...
    def save_player(self, player_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            cursor.execute(
                "SELECT id FROM players WHERE name = ?", (player_data["name"],)
            )
    ...
    def get_zone_enemies(self, zone_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute(
                "SELECT * FROM enemies WHERE zone_id = ? ORDER BY level", (zone_id,)
            )
            enemies = cursor.fetchall()
            return [dict(enemy) for enemy in enemies]
        except Exception as e:
    ...
    def get_items_by_type(self, item_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute(
                "SELECT * FROM items WHERE item_type = ? ORDER BY name", (item_type,)
            )
            items = cursor.fetchall()
            return [dict(item) for item in items]
        except Exception as e:
    ...
    def get_zone_npcs(self, zone_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute(
                "SELECT * FROM npcs WHERE zone_id = ? ORDER BY name", (zone_id,)
            )
            npcs = cursor.fetchall()
            return [dict(npc) for npc in npcs]
        except Exception as e:
    ...
    def get_quests_by_zone(self, zone_id: int) -> List[Dict[str, Any]]:
        ...
        try:
            cursor = self.connection.cursor()
            cursor.execute(
                "SELECT * FROM quests WHERE zone_id = ? ORDER BY required_level",
                (zone_id,),
            )
            quests = cursor.fetchall()
            return [dict(quest) for quest in quests]
